  ...
  <div>
    ...
    <div class="table-wrap desktop-only">
      <table class="admin-table">
        <thead><tr><th>User</th><th>Coin</th><th>Network</th><th>Amount</th><th>Status</th><th>Date</th><th>Actions</th></tr></thead>
        <tbody>
          <tr v-for="d in deposits" :key="d.id">
            <td class="fw-500">{{ d.user?.email || '—' }}</td>
            <td>{{ d.coin }}</td><td>{{ d.network }}</td>
            <td class="fw-600">{{ d.amount }}</td>
            <td><span class="badge-sm" :class="d.status">{{ d.status }}</span></td>
            <td class="fs-12 text-muted">{{ new Date(d.createdAt).toLocaleString() }}</td>
    ...
    <div class="mobile-only">
      <div v-for="d in deposits" :key="d.id" class="m-card">
        ...
        <div class="m-card-row"><span class="text-muted">Coin</span><span class="fw-500">{{ d.amount }} {{ d.coin }}</span></div>
